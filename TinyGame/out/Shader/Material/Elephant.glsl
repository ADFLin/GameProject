...
void CalcMaterialInputPS(inout MaterialInputPS input, inout MaterialParametersPS parameters)
{
	...
	float3 baseColor = tex2D(BaseTexture, float2(parameters.texCoords[0].x, 1 - parameters.texCoords[0].y));
	//input.baseColor = ( 0.5 * sin(View.gameTime) + 0.5 )* parameters.vectexColor;
	input.baseColor = s * baseColor * parameters.vectexColor /** frac( dot(parameters.worldPos, parameters.worldPos) / 100 )*/;
	//input.baseColor = float3(parameters.texCoords[0].xy, 0);
	input.metallic = 0.3;
	input.roughness = 0.83;
	input.emissiveColor = 0;
	//input.roughness = tex2D(BaseTexture, parameters.texCoords[0].xy).r * 0.9;
	input.specular = 1 * baseColor;
