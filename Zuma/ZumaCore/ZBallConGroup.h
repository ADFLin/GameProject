...
#define  MAX_NUM_COMBOINFO_TOOL 10
	...
	public:
		...
		void      applyBomb( Vec2D const& pos , float radius );
		...
		ZConBall* findNearestBall( Vec2D const& pos , float& minR2 , bool checkMask = false );
		...
		void      reset();
		//tMin > 0
		ZConBall* rayTest( Vec2D const& rPos , Vec2D const& rDir , float& tMin );

	private:
		bool      checkBallMask( ZConBall& ball , Vec2D const pos );
