...
#define RESULT_FAILED( hr ) ( hr ) != S_OK
	...
	Render::RHIDepthStencilState* D3D11System::RHICreateDepthStencilState(DepthStencilStateInitializer const& initializer)
	{
		...
		desc.DepthEnable = (initializer.depthFun != ECompareFunc::Always) || (initializer.bWriteDepth);
...
#undef CLEAR_SHADER_STATE
	...
	bool D3D11Context::determitPrimitiveTopologyUP(EPrimitive primitiveType, int num, int const* pIndices, D3D_PRIMITIVE_TOPOLOGY& outPrimitiveTopology, ID3D11Buffer** outIndexBuffer, int& outIndexNum)
	{
		if( primitiveType == EPrimitive::Quad )
		{
			int numQuad = num / 4;
		...
		else if( primitiveType == EPrimitive::LineLoop )
		{
			if( pIndices )
			{
