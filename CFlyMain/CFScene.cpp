...
namespace CFly
{
	...
	NODE* Scene::cloneNode( NODE* ptr , SceneNode* parent )
	{
		...
		EntityManager::Get().registerEntity( node );
	...
	Object* Scene::createSkyBox( char const* name , float size , bool isCubeMap )
	{
		...
		EntityManager::Get().registerEntity( obj );
	...
	void Scene::destroyNodeImpl( SceneNode* node )
	{
		...
		if ( !EntityManager::Get().removeEntity( node ) )
			return;
	...
	{
		return (SceneNode*) EntityManager::Get().extractEntityBits( id ,
	...
	Actor* Scene::createActorFromFile(char const* fileName, char const* loaderName)
	{
		...
		if ( !PluginManager::Get().load( getWorld() , actor , fileName , loaderName ) )
		{
			_destroyActor( actor );
			return nullptr;
		}
	...
	{
		return PluginManager::Get().load( getWorld() , this , fileName , loaderName , listener );
	}
