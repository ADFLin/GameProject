...
bool PropData::getString(FString& str)
{
	...
	case PROP_FLOAT:
		{
			str.format("%g", cast< float >());
			cutFloatZero(str);
		}
		return true;
	case PROP_VEC2F:
		{
			auto& value = cast<Vec2f>();
			FString temp;
			temp.format("%g %g", value.x, value.y);
			str += temp;
		}
		return true;
	case PROP_VEC3F:
		{
			auto& value = cast<Vec3f>();
			FString temp;
			temp.format("%g %g %g", value.x, value.y, value.z);
			str += temp;
		}
		return true;
	case PROP_STRING:
...
bool PropData::setValue(char const* str)
{
	...
	case PROP_VEC2F:
		{
			Vec2f value;
			int num = ::sscanf(str, "%f %f", &value.x, &value.y);
			if (num != 2)
				return false;
			cast< Vec2f >() = value;
		}
	case PROP_VEC3F:
		{
			Vec3f value;
			int num = ::sscanf(str, "%f %f %f", &value.x, &value.y, &value.z);
			if (num != 3)
				return false;
			cast< Vec3f >() = value;
		}
...
void TextPropEditor::addProp(char const* name, void* value, int sizeValue, int numSet, int const valueSet[], char const* const strSet[], unsigned flag)
{
	Prop prop;
	prop.name = name;
...
void TextPropEditor::addProp(char const* name, void* value, int sizeValue, TArrayView< ReflectEnumValueInfo const > valueSet, unsigned flag)
{
	Prop prop;
	prop.name = name;
	prop.data.setEnumData(value, sizeValue);
