...
{
	++g_dbgNumChest;
	LogDevMsg( 5, "g_dbgNumChest = %d" , g_dbgNumChest );
...
AIActionSchedule* CNPCBase::getScheduleByType( int sche )
{
	...
	LogMsg("No Schedule Define %d" , sche );
	return NULL;
}
...
void CNPCBase::maintainSchedule()
{
	...
	for( int loop = 0; loop < MAX_TASK_LOOP  ; ++loop )
	{
		if ( !checkScheduleValid() )
		{
			...
			++g_useScheduleNum;
			LogDevMsg( 5 , "%d , change Schedule = %s" , 
				g_useScheduleNum ,  getCurSchedule()->name );
		}

		if ( !getCurSchedule() )
		{
			LogDevMsg( 5 , "No Schedule run" );
			break;
		}
		...
		{
			LogDevMsg( 5 , "new Task  = %s" , curTask->name );
			startTask( *curTask );
		...
		case TKS_SUCCESS: 
			...
			break;
		case TKS_FAIL:    
			LogDevMsg( 5 , "Task Fail : %s " , curTask->name );
			done = true;
			break;
		case TKS_RUNNING:
			done = true;
		...
		{
			LogMsg( "%Max Task Loop : %s" , getCurSchedule()->name );
		}
	}
}
