...
namespace Zuma
{
	...
	void ZLevel::applyBomb( Vector2 const& pos )
	{
		for ( int i = 0 ; i < getBallGroupNum(); ++i )
		{
	...
	void ZLevel::updateMoveBall( long time )
	{
			...
			iter != mShootBallList.end();  )
		{
			...
			{
				Vector2 offset = ball->getPos() - findBall->getPos();
	...
	ZConBall* ZLevel::calcFrogAimBall( Vector2& aimPos )
	{
		float tMin = 1e8;
	...
	bool ZLevel::isInScreenRange( Vector2 const& pos )
	{
	...
	void ZLevel::createConGroup()
	{
		...
		for ( int i = 0 ; i < mLvInfo.numCurve ; ++i )
		{
			...
			if ( !loadPathVertex( mLvInfo.pathCurve[i].c_str() ,posVec ) )
			{
				...
				data.pos = 0.8f * Vector2(290,50) ; posVec.push_back( data );
				data.pos = 0.8f * Vector2(580,55) ; posVec.push_back( data );
				data.pos = 0.8f * Vector2(725,175); posVec.push_back( data );
			}
