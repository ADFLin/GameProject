...
{
	HandleManager::instance().unMarkRefHandle( this );
}

unsigned int HandledObject::getRefID()
{
	if ( m_refHandle.getID() == ERROR_ID )
		return HandleManager::instance().markRefHandle( this );
	return m_refHandle.getID();
}

int HandledObject::listenEvent( EventType type , int id )
{
	return TEventSystem::instance().connectEvent( this , type , id );
}

int HandledObject::listenEvent( EventType type , int id , FnEvent callback )
{
	return TEventSystem::instance().connectEvent( this , type , id , callback );
}

int HandledObject::listenEvent( EventType type , int id , FnMemEvent callback )
{
	return TEventSystem::instance().connectEvent( this , type , id , callback );
}

void HandledObject::sendEvent( EventType type , void* data, bool destoryData )
{
